...
public interface PartnerLifePolicyHolderMapper extends BaseMapper<PartnerLifePolicyHolder> {
    ...
    @Select("select count(*) from partner_life_policy_holder where life_policy_id in (select id from partner_life_policy where partner_id=#{partnerId} and LENGTH(id)=5)")
